...
👉 [[https://github.com/unravel-team/metaprogramming][unravel-team/metaprogramming]]
...
- [[#the-environment-setup-section][The Environment Setup section]]
  - [[#the-env-and-envsample-section-of-the-makefile][The .env and .env.sample section of the Makefile]]
...
Whenever I start a new Python project, I run through a series of steps
to standardize the project's developer experience. This project is an
attempt to short-circuit that, and also to note all the steps down for
...
These configuration files are needed for various tools to work
...
This section manages the local development infrastructure using Docker
Compose, including databases, message queues, and other services
...
This section handles deployment to Fly.io with backup and rollback
capabilities. The deployment process is designed to be safe and
reversible.
...
2. Backing up the current production image
3. Deploying the new version
4. Auto-tagging successful deployments
...
The rollback feature allows you to quickly revert to the previous
working version if a deployment fails.
...
Sometimes you need to build an image without deploying it (useful for
hot-swaps or migrations).
...
This section contains targets for maintaining the project, including
dependency upgrades and cleanup operations.
...
We use UV's built-in upgrade functionality to keep dependencies
up-to-date. This is safer than manual editing of the pyproject.toml
file.
